...
export default function ListForEachMuscles() {
	...
	return (
		<>
			<div className="mt-8 flex gap-4 overflow-x-auto rounded bg-light py-3 px-2 dark:bg-dark">
			...
			<div className="py-4">
				{muscles &&
					muscles.map((muscle: any, index: Index) => (
						<div
							className="my-4 mx-4 flex rounded bg-light p-4 dark:bg-dark"
							key={index}
						>
									...
									{muscle.secondary &&
										muscle.secondary.map(
											(
												...
												return (
													<p
														className="m-1 w-fit rounded bg-light px-2 py-1 dark:bg-dark"
														key={index}
													>
														{secondary}
													</p>
