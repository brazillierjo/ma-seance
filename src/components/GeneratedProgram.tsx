			...
			{program &&
				program.map((group: any, index: any) => {
					if (group) {
						return (
							<div
								className="my-4 rounded-md border-4 border-primary bg-lightGray p-2 dark:bg-darkGray"
								key={index}
							>
								...
								{group &&
									group.map(
										(exercice: DataType, index: Index) => {
											return (
												<div
													className="my-4 mx-4 flex rounded bg-light p-4 dark:bg-dark dark:text-white"
													key={index}
												>
